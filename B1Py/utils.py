...
class NumpyMemMapDataPipe():
    def __init__(self, channel_name, force=False, dtype='uint8', shape=(640,480,3)):
        ...
        self.shape = shape
        if force:
            self.shm = np.memmap('/dev/shm/'+channel_name,dtype=self.dtype, mode='w+',shape=shape)
        else:
            self.shm = np.memmap('/dev/shm/'+channel_name,dtype=self.dtype, mode='r+',shape=shape)

    def write(self, data):
        assert data.shape == self.shape, 'The data and the shape of the shared memory must match'  
        self.shm[:] = data
