...
export B1PY_PATH="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
...
extract_python_exe() {
    ...
    then
        ...
        # Use TeamCity build
        build_path=${B1PY_PATH}/_isaac_sim
    fi
...
update_vscode_settings() {
    ...
    ${python_exe} ${B1PY_PATH}/.vscode/tools/setup_vscode.py
}
...
while [[ $# -gt 0 ]]; do
    ...
    case "$1" in
        -i|--install)
            ...
            ${python_exe} -m pip install -e ${B1PY_PATH}
            shift # past argument
            ;;
        -f|--format)
        ...
        #     shift # past argument
        #     ${isaacsim_exe} --ext-folder ${B1PY_PATH}/source/extensions $@
        #     # exit neatly
        #     break
        #     ;;
        ...
        -s|--sim)
            ...
            python_exe=$(extract_python_exe)
            # install the rl-frameworks specified
            ${python_exe} ${B1PY_PATH}/B1Py/sim/isaac/isaacsim_node.py 
            # exit neatly
            break
            ;;
